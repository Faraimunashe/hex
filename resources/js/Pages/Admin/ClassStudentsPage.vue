  ...
  <div class="p-6 bg-gray-100 min-h-screen space-y-6">
    <div class="mb-4">
      <h1 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
        <i class="fas fa-users text-blue-600"></i> {{ module.name }}
      </h1>
      <p class="text-sm text-gray-500">Department: {{ module.department }}</p>
    </div>
...
export default {
  ...
  setup(props) {
    ...
    watch(search, (value) => {
      router.get(`/admin/classes/${props.module.id}`, { search: value }, {
        preserveState: true,
        replace: true,
      });
    });
