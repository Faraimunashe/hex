  ...
  <div class="p-6 bg-gray-100 min-h-screen space-y-6">
    <div class="mb-4">
      <h1 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
        <i class="fas fa-user-graduate text-blue-600"></i> Students
      </h1>
    ...
    <div class="grid gap-4">
      ...
      >
        <div class="flex items-center gap-4 w-full md:w-2/3">
          ...
          <div>
            <p class="text-lg font-semibold text-gray-800">{{ student.firstnames }} {{ student.surname }}</p>
            <p class="text-sm text-gray-500">
              {{ student.regnum }} • {{ student.gender }} • {{ student.department?.name }} • {{ student.user?.email }}
            </p>
          </div>
        </div>

        <div class="flex items-center gap-2 mt-4 md:mt-0 w-full md:w-1/3 justify-end">
          <Link :href="`/admin/students/${student.id}`" class="text-blue-600 hover:text-blue-800 p-2">
            <i class="fas fa-eye"></i>
          </Link>
          <Link :href="`/admin/students/${student.id}/edit`" class="text-blue-600 hover:text-blue-800 p-2">
            <i class="fas fa-pen"></i>
          </Link>
          <button @click="openDeleteModal(student.id)" class="text-red-600 hover:text-red-800 p-2">
    ...
    <div v-if="students.data.length === 0" class="text-center text-gray-500 py-12">
      <i class="fas fa-user-graduate text-4xl text-blue-200 mb-4"></i>
    ...
    <div v-if="isDeleteModalOpen" class="fixed inset-0 backdrop-blur-sm bg-white/30 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-sm">
        ...
        <div class="flex justify-end gap-2">
          ...
          <button @click="handleDelete(selectedStudentId)" class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700">
            Delete
          </button>
        </div>
      </div>
...
export default {
  ...
  setup(props) {
    ...
    watch(search, (value) => {
      router.get('/admin/students', { search: value }, {
        preserveState: true,
        replace: true
      });
    });
