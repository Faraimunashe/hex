  ...
  <div class="p-6 bg-gray-100 min-h-screen space-y-6">
    <div class="flex items-center justify-between">
      <h1 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
        <i class="fas fa-calendar-alt text-blue-600"></i> My Timetable
      </h1>
      <div class="flex items-center gap-2">
        ...
        <a href="/lecturer/timetable/pdf" class="px-3 py-2 bg-indigo-600 text-white rounded text-sm hover:bg-indigo-700">
      ...
      <div v-for="(day, date) in groupedEvents" :key="date" class="bg-white rounded-xl shadow">
        ...
        <div class="p-4 space-y-3">
          <div v-for="ev in day" :key="ev.id" class="flex items-center justify-between border rounded-lg p-3 hover:bg-gray-50">
            ...
            <Link :href="`/lecturer/timetables/${ev.id}`" class="px-3 py-1 bg-blue-600 text-white rounded text-sm hover:bg-blue-700">View</Link>
          </div>
        </div>
      </div>
...
export default {
  ...
  setup(props) {
    ...
    watch(search, (value) => {
      router.get('/lecturer/timetables', { search: value }, { preserveState: true, replace: true });
    });
